...
public class GetAllWorkOrderHeaderPagedListHandlerTests
{
    [Fact]
    public async Task Handle_WhenRepositorySucceeds_ReturnsMappedPagedHeaders()
    {
        ...
        [
            ...
            new WorkOrderHeader
            {
                WorkOrderHeaderId = 21,
                AGSEAMWOID = "WO-21",
                HeaderTitle = "Replace belt"
            }
        ], 2, 10, 12);

        unitOfWorkMock.SetupGet(unitOfWork => unitOfWork.WorkOrderHeaderRepository)
            .Returns(repositoryMock.Object);
        repositoryMock.Setup(repository => repository.GetAllPagedList(2, 10))
            .ReturnsAsync(pagedResult);

        var handler = new GetAllWorkOrderHeaderPagedListHandler(mapper, unitOfWorkMock.Object);

        var result = await handler.Handle(2, 10);
        ...
        Assert.Contains(result.Data.Items, header => header.WorkOrderHeaderId == 20 && header.HeaderTitle == "Inspect pump");
        Assert.Contains(result.Data.Items, header => header.WorkOrderHeaderId == 21 && header.HeaderTitle == "Replace belt");
    ...
    [Fact]
    public async Task GetAllWorkOrderHeaderPagedList_WhenCalled_DelegatesToHandler()
    {
        var unitOfWorkMock = new Mock<IUnitOfWork>(MockBehavior.Strict);
        ...
        getAllPagedListHandlerMock.Setup(handler => handler.Handle(2, 10))
        ...
        var result = await service.GetAllWorkOrderHeaderPagedList(2, 10);

        Assert.Same(expectedResponse, result);
        getAllPagedListHandlerMock.Verify(handler => handler.Handle(2, 10), Times.Once);
    }
    ...
    public async Task Handle_WhenRepositoryThrows_ReturnsErrorResponse()
    {
        ...
        var repositoryMock = new Mock<IWorkOrderHeaderRepository>(MockBehavior.Strict);
        var mapper = new MapperlyMapper();
        var expectedException = new InvalidOperationException("paged header read failed");
