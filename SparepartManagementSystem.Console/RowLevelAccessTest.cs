...
{
    public void TestRowLevelAccess()
    {
        ...
        foreach (var parm in parms)
        {
            ...
            System.Console.WriteLine($"parm = {parm}, expected output: {string.Join(", ", filteredList)}");

            List<string> secondList = [];
            secondList = secondList.Count != 0 ? secondList.FilterStringsByPrefix(parm).ToList() : ["*"];
    
            System.Console.WriteLine($"parm = {parm}, expected output: {string.Join(", ", secondList)}");
        }
    }
}